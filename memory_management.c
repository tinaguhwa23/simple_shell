 ...
 *
 * @source: This a pointer to the memory previously allocated.
...
void *_realloc(void *source, unsigned int _size, unsigned int size)
{
	...
	if (size < _size)
		_memocpy(destination, source, size);
	else
		_memocpy(destination, source, _size);
...
/**
 * _reallocdptr - This reallocates a memory block of a double pointer.
