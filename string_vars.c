 ...
 *
 * @h: This is a head of linked list
 * @in: input string
 * @data: data structure
 * Return: no return
 */
					 ...
					 char *input, char *new_input, int nlen)
{
	...
	for (j = i = 0; i < nlen; i++)
	{
		if (input[j] == '$')
		{
			if (!(indx->var_length) && !(indx->len_val))
			{
				new_input[i] = input[j];
				j++;
			}
			else if (indx->var_length && !(indx->len_val))
			{
...
/**
 * replace_var - This calls functions to replace string into vars
 *
 * @input: input string
